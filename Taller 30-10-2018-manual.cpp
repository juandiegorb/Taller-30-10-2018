...
void crear(struct tienda tien[4]){	
	for(int i = 0; i <4; i++){
		tien[i].p.cod = i+1;
		printf("Ingrese el nombre del producto %d: ", tien[i].p.cod);
		fflush(stdin);
		gets(tien[i].p.nom);
	}
	system("cls");	
...
void calcular(struct tienda tien[4]){
	int n, seleccion;	
	// For para las tiendas totales	
	for(int i=0;i<4;i++){
		printf("Hay 4 productos\n");	
		for(int p = 0; p < 4; p++ ){
			printf("%d. %s\n", tien[p].p.cod, tien[p].p.nom);
		}
		printf("Tienda %d\n", i+1);
...
void mostrar(struct tienda tien[4]){
...
void mostrartotal(struct tienda tiend[4]){
	float totalfinal[4];
	//TOTAL FINAL
	...
	for(int j=0;j<4;j++){
		...
		printf("Producto %d. %s total vendido  es: %.2f\n", tien[j].p.cod, tien[j].p.nom, totalfinal[j]);			
	};
	getch();
}
